...
const defaultInputSize = inject("defaultInputSize", "xlarge");

const computedFontFace = computed(() => {
  return props.fontFace ? props.fontFace : unref(defaultFontFace)
    ? unref(defaultFontFace)
    : "circularSTD";
});

const computedInputSize = computed(() => {
  return props.size || unref(defaultInputSize) ? un
})
